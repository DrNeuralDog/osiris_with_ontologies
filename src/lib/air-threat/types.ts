...
export function reportGroup(type: string) { return type.startsWith('HEARD_') ? 'Acoustic' : type.includes('DRONE') ? 'Drone' : type.includes('MISSILE') ? 'Missile' : type === 'ROCKET_ALERT' ? 'Rocket' : type.includes('BOMB') ? 'Guided bomb' : type === 'AIR_DEFENSE_ACTIVITY' ? 'Air defence' : type === 'INTERCEPTION_REPORT' ? 'Interception' : type === 'EXPLOSION_REPORT' ? 'Explosion' : ['AIR_RAID_ALERT', 'PRE_ALERT', 'ALL_CLEAR'].includes(type) ? 'Official alerts' : 'Other conflict'; }
