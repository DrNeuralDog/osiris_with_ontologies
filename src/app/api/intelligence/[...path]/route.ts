...
export async function proxyIntelligence(req: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  const allowed = req.method === 'GET' ? new RegExp(`^(air-threat/(state|areas)|timeline/(state|events|coverage|chunk)|summary|objects/${uuid}/context|policies|sources|sources/[a-zA-Z0-9:._-]{1,160}/samples|correlations|correlations/${uuid})$`).test(route) : req.method === 'POST' && new RegExp(`^correlations/${uuid}/dismiss$`).test(route);
  ...
  const params = new URL(req.url).searchParams, keys = new Set(route.startsWith('air-threat/')?['at','bbox','admin','limit']:route.startsWith('timeline/')?['at','from','to','bbox','domains','limit','cursor']:['scope', 'category', 'status', 'limit', 'cursor', 'type', 'object_id']);
  for (const [key, value] of params) if (!keys.has(key) || params.getAll(key).length > 1 || value.length > (route.startsWith('timeline/')?600:200)) return NextResponse.json({ error: 'Invalid query' }, { status: 400 });
  try {
