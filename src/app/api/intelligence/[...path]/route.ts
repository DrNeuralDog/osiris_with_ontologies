...
export async function proxyIntelligence(req: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  const allowed = req.method === 'GET' ? new RegExp(`^(timeline/(state|events|coverage|chunk)|summary|objects/${uuid}/context|policies|sources|sources/[a-zA-Z0-9:._-]{1,160}/samples|correlations|correlations/${uuid})$`).test(route) : req.method === 'POST' && new RegExp(`^correlations/${uuid}/dismiss$`).test(route);
  ...
  const params = new URL(req.url).searchParams, keys = new Set(route.startsWith('timeline/')?['at','from','to','bbox','domains','limit','cursor']:['scope', 'category', 'status', 'limit', 'cursor', 'type', 'object_id']);
