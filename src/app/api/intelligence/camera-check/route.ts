import { fetchCameraImage } from '@/lib/cctv-image';
...
interface Check { id: string; camera_id: string; status: string; checked_at: string; error: string | null; next_check_at: string; snapshot_status: string; stream_status: string }
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (prior.ok) { const saved = await prior.json(); if (saved.next_check_at && Date.parse(saved.next_check_at) > Date.now()) return NextResponse.json({ id: sourceId, camera_id: id, status: saved.status, checked_at: saved.last_checked_at, error: saved.last_error_category, next_check_at: saved.next_check_at, snapshot_status: saved.camera_media?.snapshot_status || 'UNKNOWN', stream_status: saved.camera_media?.stream_status || 'UNKNOWN', cached: true }); }
    ...
    if (!camera.feed_url || camera.feed_url === camera.stream_url && camera.stream_type !== 'jpg') return NextResponse.json({ status: 'UNKNOWN', snapshot_status: 'UNKNOWN', stream_status: camera.external_url && !camera.stream_url ? 'EXTERNAL_ONLY' : 'UNKNOWN', camera_id: id, error: 'No separate catalog snapshot is available. Video playback requires a browser check.' });
    const feed = String(camera.feed_url);
    const proxyTarget = feed.startsWith('/api/cctv/proxy?') ? new URL(feed, 'http://catalog.invalid').searchParams.get('url') : null;
    if(feed.startsWith('/') && !proxyTarget) return NextResponse.json({ status: 'UNKNOWN', snapshot_status: 'UNKNOWN', stream_status: 'UNKNOWN', camera_id: id, error: 'This provider snapshot must be checked in the browser.' });
    const target = new URL(proxyTarget || feed);
    ...
    try {
      const proxied = proxyTarget ? await fetchCameraImage(target.href) : null;
      const frame = proxied ? new Response(new Uint8Array(proxied.data), {status: proxied.status, headers: {'Content-Type':proxied.contentType}}) : await safeFetch(target.href, { maxRedirects: 0, signal: AbortSignal.timeout(8000), headers: { Accept: 'image/*' } }); code = frame.status;
    ...
    const saved = await fetch(`${base}/intelligence/source-reports`, { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(5000), headers: { 'Content-Type': 'application/json', 'X-Intelligence-Key': process.env.INTELLIGENCE_REPORT_KEY || 'osiris-local-reports' }, body: JSON.stringify({ reports: [{ source: { id: sourceId, name: String(camera.name || id).slice(0, 160), category: 'cctv', scope: 'camera', endpoint: id }, sample: { camera_media: {snapshot_status: ok ? 'SNAPSHOT_AVAILABLE' : 'SNAPSHOT_UNAVAILABLE', stream_status: camera.external_url && !camera.stream_url ? 'EXTERNAL_ONLY' : 'UNKNOWN'}, ok, latency_ms: Date.now() - start, http_status: code, error_category: error, record_count: ok ? 1 : 0 } }] }) });
    if (!saved.ok) return NextResponse.json({ error: 'Camera checked but health could not be persisted' }, { status: 502 });
    const result: Check = { id: sourceId, camera_id: id, status: ok ? 'SNAPSHOT_AVAILABLE' : 'CHECK_FAILED', snapshot_status: ok ? 'SNAPSHOT_AVAILABLE' : 'SNAPSHOT_UNAVAILABLE', stream_status: camera.external_url && !camera.stream_url ? 'EXTERNAL_ONLY' : 'UNKNOWN', checked_at: new Date().toISOString(), error: error || null, next_check_at: new Date(Date.now() + (ok ? 600000 : 120000)).toISOString() };
