...
describe('catalog-bound camera probes', () => {
  beforeEach(() => { vi.clearAllMocks(); vi.stubGlobal('fetch', vi.fn().mockResolvedValue(Response.json({}, { status: 404 }))); vi.mocked(catalog).mockResolvedValue(Response.json({ cameras: [] })); });
  afterEach(() => vi.unstubAllGlobals());
  it('rejects arbitrary URLs and unknown IDs without a camera request', async () => { expect((await POST(new Request('http://localhost/x?id=x&url=http://localhost', { method: 'POST' }))).status).toBe(400); expect((await POST(request('unknown'))).status).toBe(404); expect(safeFetch).not.toHaveBeenCalled(); });
  it('leaves iframe availability unknown', async () => { vi.mocked(catalog).mockResolvedValue(Response.json({ cameras: [{ id: 'video', stream_type: 'iframe', stream_url: 'https://example.com' }] })); expect((await (await POST(request('video'))).json()).status).toBe('UNKNOWN'); expect(safeFetch).not.toHaveBeenCalled(); });
  it('requires image bytes and persists the real outcome', async () => {
    vi.mocked(catalog).mockResolvedValue(Response.json({ cameras: [{ id: 'frame', name: 'Camera', feed_url: 'https://known.example/frame.jpg' }] }));
