...
export default function InvestigationActions({ entity, onInvestigate }: { entity: Record<string, unknown>; onInvestigate: (seed: InvestigationSeed, intent: InvestigationIntent) => void }) {
  const seed = mapEntitySeed(entity) || (['company','person','country'].includes(String(entity.type)) ? mapInvestigationSeed(entity) : null);
