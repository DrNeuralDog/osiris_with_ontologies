    ...
    <div className={`glass-panel flex flex-col overflow-hidden pointer-events-auto shrink-0 ${full?'w-full h-full':'h-[500px] max-h-[80vh] resize-y'}`}>
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2.5 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.3)] hover:bg-[var(--hover-accent)] transition-colors">
        <button onClick={()=>setExpanded(!expanded)} className="flex items-center gap-2 flex-1">
          ...
          <span className="gotham-tag gotham-tag--info" style={{fontSize:'9px',padding:'1px 5px'}}>{devices.length}{" "}{uiText("DEVS")}</span>
          {scanning&&<span className="gotham-tag" style={{fontSize:'9px',padding:'1px 5px',background:'rgba(0,230,255,0.1)',color:'var(--cyan-primary)'}}>{uiText("SCANNING")}</span>}
        </button>
        <div className="flex items-center gap-2">
          <motion.button whileTap={{scale:0.95}} onClick={scan} disabled={scanning||!btOk}
            ...
            {scanning?<BluetoothSearching className="w-3 h-3 animate-pulse"/>:<Bluetooth className="w-3 h-3"/>}{scanning?'...':uiText("SCAN")}
          </motion.button>
          {geoCount>0&&<motion.button whileTap={{scale:0.95}} onClick={placeOnWorldMap}
            ...
            <MapPin className="w-3 h-3"/>{uiText("VIEW ON MAP")}{" "}</motion.button>}
          <button onClick={()=>setFull(!full)} className="p-1.5 -m-0.5 rounded text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-white/10 transition-colors" title={uiText("Full Screen")}>
      ...
      <AnimatePresence>{expanded&&(
        <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={{duration:0.2}} className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex items-center px-2 py-1.5 shrink-0 gap-0.5 border-b border-[rgba(255,255,255,0.04)]" style={{background:'rgba(0,0,0,0.2)'}}>
            {([['scan','DEVICES',Scan],['intel','INTEL',Globe],['log','LOG',Terminal]] as const).map(([id,label,Icon])=>(
              <button key={id} onClick={()=>setView(id as any)} className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-[9px] font-mono font-bold tracking-[0.12em] transition-all flex-1 justify-center"
                ...
                <Icon className="w-3 h-3"/>{uiText(String(label))}
              </button>
            ))}
          </div>
              ...
              <div className="flex-1 overflow-y-auto styled-scrollbar px-2.5 py-2.5 space-y-2">
                {devices.length===0&&!scanning&&btOk&&(
                  <div className="flex flex-col items-center justify-center py-8 opacity-40">
                    ...
                    <p className="text-[10px] font-mono text-[var(--text-muted)] text-center tracking-wider">{uiText("Hit")}{" "}<span className="text-[var(--cyan-primary)]">{uiText("SCAN")}</span>{" "}{uiText("to acquire targets")}<br/>
                      <span className="text-[9px] opacity-50">{uiText("Auto-vacuum · GPS-tagged · Vault")}</span>
                    </p>
                  </div>
                )}
                {!btOk&&<div className="p-2 rounded-lg" style={{background:'rgba(255,61,61,0.04)',border:'1px solid rgba(255,61,61,0.08)'}}><span className="text-[9px] font-mono text-[#FF3D3D]">{uiText("Web Bluetooth unavailable — use Chrome/Edge on localhost")}</span></div>}
                    ...
                    <div className="rounded-xl overflow-hidden transition-all" style={{background:dev.connected?`linear-gradient(135deg,${dev.color}06,${dev.color}02)`:'rgba(255,255,255,0.015)',border:`1px solid ${dev.connected?dev.color+'15':'rgba(255,255,255,0.04)'}`,boxShadow:dev.connected?`0 0 16px ${dev.color}08`:'none'}}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {dev.probing?(
                            ...
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              <span className="text-[9px] font-mono text-[var(--text-muted)]">{dev.serviceCount}{uiText("svcs ·")}{" "}{dev.probeMs}{uiText("ms")}</span>
                            </div>
                          )}
                        </div>
                      ...
                      <AnimatePresence>{isE&&!dev.probing&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="px-3 pb-3" style={{borderTop:'1px solid rgba(255,255,255,0.03)'}}>
                        ...
                        <div className="mt-2 mb-2.5 rounded-lg p-2.5" style={{background:'rgba(0,0,0,0.25)',border:'1px solid rgba(255,255,255,0.03)'}}>
                          <div className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.2em] mb-1.5 font-bold">{uiText("DEVICE INTEL")}</div>
                          <div className="space-y-1">
                            ...
                            <div className="flex gap-2"><span className="text-[9px] font-mono text-[var(--text-muted)] w-[65px] shrink-0 tracking-wider">{uiText("Extracted")}</span><span className="text-[9px] font-mono text-[var(--text-muted)]">{dev.serviceCount}{uiText("svcs ·")}{" "}{dev.charCount}{uiText("chars ·")}{" "}{dev.gattDump.length}{uiText("vals ·")}{" "}{dev.totalBytes}B</span></div>
                          </div>
                        </div>

                        {/* Services */}
                        {dev.services.length>0&&<div className="flex flex-wrap gap-1 mb-2.5">{dev.services.map((s,i)=><span key={i} className="text-[9px] font-mono px-1.5 py-0.5 rounded tracking-wider" style={{background:`${dev.color}06`,color:`${dev.color}70`}}>{uiText(String(s))}</span>)}</div>}

                        {/* GATT Dump */}
                        {dev.gattDump.length>0&&<div className="mb-2.5"><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-[0.15em] mb-1 font-bold">{uiText("VACUUM DUMP (")}{dev.gattDump.length})</div><div className="max-h-[120px] overflow-y-auto styled-scrollbar rounded-lg" style={{background:'rgba(0,0,0,0.3)'}}>
                        ...
                        <div className="flex gap-1.5 flex-wrap">
                          {dev.bt&&(dev.connected?(<>
                            ...
                            <motion.button whileTap={{scale:0.95}} onClick={()=>disconnect(dev)} className="py-1.5 px-2.5 rounded-lg text-[9px] font-mono font-bold flex items-center gap-1" style={{background:'rgba(255,61,61,0.05)',color:'#FF6B6B',border:'1px solid rgba(255,61,61,0.06)'}}><Unplug className="w-3 h-3"/>{uiText("DROP")}</motion.button>
                          </>):(<>
                        ...
                        <AnimatePresence>{isG&&(<motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className="overflow-hidden"><div className="mt-2.5 pt-2.5" style={{borderTop:`1px solid ${dev.color}08`}}>
                          <div className="flex items-center justify-between mb-2"><span className="text-[9px] font-mono font-bold tracking-[0.12em]" style={{color:dev.color}}>{uiText("GATT TREE")}</span><button onClick={()=>{setGattTarget(null);setGattSvcs([]);}}><X className="w-2.5 h-2.5 text-[var(--text-muted)]"/></button></div>
                          {gattLoading?<div className="flex items-center justify-center py-3 gap-1.5"><div className="w-3.5 h-3.5 border-2 rounded-full animate-spin" style={{borderColor:dev.color,borderTopColor:'transparent'}}/><span className="text-[9px] font-mono animate-pulse" style={{color:dev.color}}>{uiText("Enumerating...")}</span></div>
              ...
              <div className="flex-1 overflow-y-auto styled-scrollbar p-2.5 space-y-2.5">
                ...
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center justify-between" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                    <div className="flex items-center gap-1.5"><Globe className="w-3 h-3 text-[#80DEEA]"/><span className="text-[9px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">{uiText("NETWORK RECON")}</span></div>
                  ...
                  {!netIntel&&<div className="px-3 py-4 text-center"><span className="text-[9px] font-mono text-[var(--text-muted)] opacity-30">{uiText("Tap PROBE to scan local network")}</span></div>}
                  {netIntel&&<div className="p-2.5 space-y-2">
                    {netIntel.localIPs.length>0&&<div><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">{uiText("LOCAL IPs")}</div>{netIntel.localIPs.map((ip,i)=><div key={i} className="text-[10px] font-mono text-[#80DEEA] pl-2">{ip}</div>)}</div>}
                    ...
                    {netIntel.openPorts.length>0&&<div><div className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider mb-1 font-bold">{uiText("OPEN PORTS")}</div>{netIntel.openPorts.map((p,i)=><div key={i} className="flex items-center gap-2 pl-2 py-0.5"><span className="text-[9px] font-mono font-bold text-[var(--alert-green)]">:{p.port}</span><span className="text-[9px] font-mono text-[var(--text-muted)]">{p.service}</span></div>)}</div>}
                  </div>}
                </div>
                {/* Export */}
                <div className="rounded-xl overflow-hidden" style={{background:'rgba(255,255,255,0.015)',border:'1px solid rgba(255,255,255,0.04)'}}>
                  <div className="px-3 py-2 flex items-center gap-1.5" style={{borderBottom:'1px solid rgba(255,255,255,0.03)'}}><Database className="w-3 h-3 text-[#FFB800]"/><span className="text-[9px] font-mono font-bold tracking-[0.12em] text-[var(--text-primary)]">{uiText("VAULT & EXPORT")}</span></div>
                  <div className="p-2.5 space-y-1.5">
                    <div className="text-[9px] font-mono text-[var(--text-muted)]">{vaultCount}{" "}{uiText("device")}{vaultCount!==1?'s':''} · {bytes>1024?`${(bytes/1024).toFixed(1)}KB`:`${bytes}B`}</div>
            ...
            {view==='log'&&(
              <div className="flex-1 flex flex-col overflow-hidden">
                <div className="flex items-center justify-between px-3 py-1 shrink-0" style={{background:'rgba(0,0,0,0.3)',borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                  <span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{pkts.length}{" "}{uiText("pkts ·")}{" "}{bytes>1024?`${(bytes/1024).toFixed(1)}KB`:`${bytes}B`}</span>
                  <div className="flex items-center gap-1.5">
                    <button onClick={()=>setPaused(!paused)} className={`text-[9px] font-mono tracking-wider px-1 py-0.5 rounded flex items-center gap-0.5 ${paused?'text-[#FFB800]':'text-[var(--text-muted)]'}`}>{paused?<><Pause className="w-2 h-2"/>{uiText("PAUSE")}</>:<><Play className="w-2 h-2"/>{uiText("LIVE")}</>}</button>
                    <button onClick={()=>{setPkts([]);setBytes(0);pid.current=0;}} className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider px-1 py-0.5 rounded hover:bg-white/5 flex items-center gap-0.5"><Trash2 className="w-2 h-2"/>{uiText("CLR")}</button>
                  </div>
                </div>
                <div ref={logEl} className="flex-1 overflow-y-auto styled-scrollbar font-mono" style={{background:'#020406'}}>
                  {pkts.length===0?<div className="flex items-center justify-center h-full opacity-15"><span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{uiText("Scan to see packets...")}</span></div>
          ...
          <div className="flex items-center justify-between px-3 py-1.5 shrink-0" style={{borderTop:'1px solid rgba(255,255,255,0.03)',background:'rgba(0,0,0,0.2)'}}>
            <div className="flex items-center gap-1.5"><div className={`w-1.5 h-1.5 rounded-full ${btOk?'bg-[var(--alert-green)]':'bg-[#FF3D3D]'} ${scanning?'animate-pulse':''}`}/><span className="text-[9px] font-mono text-[var(--text-muted)] tracking-wider">{scanning?uiText("SCANNING"):liveCount>0?`${liveCount} LIVE`:'STANDBY'} · {upStr}</span></div>
