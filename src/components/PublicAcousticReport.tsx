...
export default function PublicAcousticReport() {
 const {t:uiText}=useUILocale();
    ...
    return <details className="border-t border-slate-700 pt-2"><summary>{uiText("Retain a public heard-explosion report")}</summary><p>{uiText("Transcribe a real public source only. This is an unverified PUBLIC REPORT; it does not identify a weapon or sound source.")}</p><form onSubmit={e => { e.preventDefault(); void save(); }} className="space-y-1">{[['Public source URL', url, setUrl], ['Source record ID', record, setRecord], ['Reported at (ISO timezone)', at, setAt], ['Report latitude,longitude', point, setPoint]].map(([label, value, set]) => <label key={String(label)} className="block">{String(label)}<input required maxLength={500} aria-label={String(label)} className="w-full bg-slate-900 border border-slate-600 p-1" value={value as string} onChange={e => (set as (v: string) => void)(e.target.value)}/></label>)}<label className="block">{uiText("Report precision")}<select aria-label={uiText("Report precision")} className="bg-slate-900" value={precision} onChange={e => setPrecision(e.target.value)}>{['LOCALITY', 'DISTRICT', 'REGION', 'APPROXIMATE', 'EXACT_SOURCE_COORDINATE'].map(p => <option key={p}>{p}</option>)}</select></label><label className="block">{uiText("Original description")}<textarea required maxLength={2000} aria-label={uiText("Original description")} className="w-full bg-slate-900" value={description} onChange={e => setDescription(e.target.value)}/></label><button disabled={busy} className="underline">{uiText("Retain public report")}</button></form>{message && <p role="status">{message}</p>}</details>;
}
