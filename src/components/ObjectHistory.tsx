...
export default function ObjectHistory({ objectId, objectType, objectName, onMap, onJump }: { objectId: string; objectType: string; objectName: string; onMap?: (focus: InvestigationMapFocus) => void; onJump?: () => void }) {
  ...
  return <section className="space-y-2 text-xs" aria-label={uiText("Object history")}><h4 className="tracking-widest text-[var(--gold-primary)]">{uiText("HISTORY / TIMELINE")}</h4>
    <form onSubmit={e => { e.preventDefault(); void load(); }} className="space-y-2"><label className="block">{uiText("From")}{" "}<input aria-label={uiText("History from")} type="datetime-local" className="w-full bg-[var(--bg-secondary)] p-1" value={from} onChange={e => setFrom(e.target.value)} /></label><label className="block">{uiText("To")}{" "}<input aria-label={uiText("History to")} type="datetime-local" className="w-full bg-[var(--bg-secondary)] p-1" value={to} onChange={e => setTo(e.target.value)} /></label><label className="block"><input type="checkbox" checked={locations} onChange={e => setLocations(e.target.checked)} />{" "}{uiText("Location observations only")}</label><button disabled={busy} className={button}>{uiText("Apply / refresh")}</button></form>
    <p className="text-[10px] text-slate-400">{uiText("Default: last 90 days. Maximum range: 366 days. Local time shown. Unknown observation time is labelled received.")}</p>
    {onMap && points.length > 1 && <button className={button} onClick={() => onMap({ ...points[points.length - 1], objectId, points, track: true, label: `Retained positions · ${points.length} loaded observations` })}>{uiText("Show loaded track on map")}</button>}
    {error && <p role="alert" className="text-red-300">{error}</p>}{busy && <p role="status">{uiText("Loading history…")}</p>}
    {!busy && !items.length && <p className="text-slate-400">{uiText("No retained observations yet for this range. Telemetry begins after the object is registered; source timestamps are required.")}</p>}
    {items.map(o => <article key={o.id} aria-current={replay.selection?.id===o.id?"true":undefined} className="aria-[current=true]:bg-cyan-900/30 border-l-2 border-[var(--gold-primary)]/40 pl-2 space-y-1 py-2"><div className="text-cyan-300">{displayTime(o.timeline_at)}{o.time_basis === 'received' ? ' · RECEIVED' : ''}</div><strong className="break-words">{o.event_type}</strong><div>{observationEvidenceLabel(o)} · {replay.state.mode==='replay'?(historicalItem(o)?replayFreshness(historicalItem(o)!,replay.state.at,replay.policies):'UNKNOWN')+' relative to replay time':o.freshness}</div><div>{o.source_id}{" "}{uiText("· Confidence:")}{" "}{confidenceLabel(o.confidence)}</div>{o.lat != null && o.lon != null && <button className={button} disabled={!onMap} onClick={() => onMap?.({ lat: o.lat!, lng: o.lon!, objectId, label: `${o.event_type} · ${displayTime(o.observed_at)}` })}>{uiText("Locate")}{" "}{o.lat.toFixed(4)}, {o.lon.toFixed(4)}</button>}<button className={button} onClick={()=>jump(o)}>{uiText("Jump to this time on global timeline")}</button><details><summary className="cursor-pointer">{uiText("Properties & provenance")}</summary><pre className="whitespace-pre-wrap break-words text-[10px]">{JSON.stringify(o.data, null, 2)}</pre><AddToCaseButton reference={{kind:'observation',id:o.id}}/><EvidenceView items={o.provenance} /></details></article>)}
    {cursor && items.length < 500 && <button className={button} disabled={busy} onClick={() => void load(true)}>{uiText("Load next 50")}</button>}{items.length >= 500 && <p>{uiText("500 events loaded. Narrow the time range to continue.")}</p>}
  </section>;
}
