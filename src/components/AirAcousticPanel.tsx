...
export default function AirAcousticPanel({ report }: {
  ...
  const {t:uiText}=useUILocale();
    ...
    return <details className="border-t border-slate-700 pt-2"><summary>{uiText("Estimated audibility · optional scenario")}</summary><p>{uiText("SIMPLIFIED ACOUSTIC MODEL. A reported location alone cannot determine audibility. No inverse source localization.")}</p><label className="block py-2"><input type="checkbox" checked={assumed} onChange={e => setAssumed(e.target.checked)}/>{" "}{uiText("Use explicit assumed sound levels; not source measurements")}</label>
 ...
 <div className="flex flex-wrap gap-2 my-2"><button disabled={busy || !assumed || !reference || !distance || !noise} onClick={() => void estimate()} className="underline disabled:opacity-40">{uiText("Estimate scenario")}</button><button disabled={busy || replay.state.mode === 'replay'} onClick={() => void loadWeather()} className="underline disabled:opacity-40">{uiText("Retain nearby model weather")}</button><button className="underline" onClick={() => air.setAcoustic(null)}>{uiText("Clear acoustic overlay")}</button></div>
 {error && <p role="status">{error}</p>}{air.acoustic && <div><AddToCaseButton analysis={{kind:'acoustic',parameters:{observation_id:report.id,at:replay.state.mode==='replay'?new Date(replay.state.at).toISOString():undefined,scenario:assumed,reference_db:Number(reference),reference_m:Number(distance),background_db:Number(noise)}}}/><strong>{air.acoustic.status}{" "}{uiText("· HIGH UNCERTAINTY")}</strong><p>{uiText("Precision:")}{" "}{air.acoustic.location_precision} · {air.acoustic.uncertainty}</p><p>{air.acoustic.weather_state || 'Weather not evaluated'} · {air.acoustic.weather_timestamp || 'No weather time'}</p><p>{uiText("Terrain:")}{" "}{air.acoustic.terrain_state}{" "}{uiText("· Land cover:")}{" "}{air.acoustic.land_cover_state}</p>{air.acoustic.zones.map(z => <p key={z.label}>{uiText(String(z.label))}{" "}{uiText("· scenario only")}</p>)}<p>{air.acoustic.model_version}</p><EvidenceView items={air.acoustic.provenance}/><ul className="list-disc pl-4">{air.acoustic.assumptions.map(v => <li key={v}>{v}</li>)}</ul></div>}</details>;
}
