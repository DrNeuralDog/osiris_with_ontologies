...
export default function WorldLayers({mapRef,ready,layers,replayAt,onInvestigate}:Props){
 ...
 return <aside aria-label={uiText("World data layers")} className="absolute left-16 bottom-28 z-[230] w-72 max-w-[calc(100vw-120px)] max-h-[55vh] overflow-auto rounded border border-amber-300/30 bg-slate-950/95 p-3 text-[11px] font-mono text-slate-200 shadow-xl">
  ...
  {wxEnabled&&<><p><a href="https://open-meteo.com/" target="_blank" rel="noreferrer">Open-Meteo</a>{" "}{uiText("· DERIVED · model samples")}</p>{field&&<p>{uiText(String(field.label))}: <span className="text-blue-300">{field.min}</span> → <span className="text-orange-300">{field.max} {field.unit}</span></p>}{layers.wx_wind&&<p>{uiText("Arrows: wind toward · m/s; gusts in details")}</p>}</>}
