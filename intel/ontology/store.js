...
const {writeObservation,stable}=require('../intelligence/history');
const {mergeIntelligence}=require('../intelligence/identity');
...
class OntologyStore {
  ...
  async evidence(db, objectId, linkId, evidence, snapshot) {
    for (const p of evidence) {
      ...
      const fingerprint = createHash('sha256').update(stable([p.provider, p.source_id, p.source_record_id, p.url, p.observed_at, p.confidence, p.kind, metadata])).digest('hex');
      await db.query(`INSERT INTO ontology_provenance(id,object_id,link_id,provider,source_id,url,observed_at,fetched_at,confidence,kind,metadata,fingerprint,source_record_id,extraction_method,confidence_basis)
      ...
      [randomUUID(), objectId, linkId, p.provider, p.source_id, p.url, p.observed_at, p.fetched_at, p.confidence, p.kind, metadata, fingerprint,p.source_record_id||null,p.extraction_method||null,p.confidence_basis||null]);
    }
  }
  ...
  async putObject(db, raw) {
    ...
    const previous=ids.length?(await db.query('SELECT properties,canonical_name FROM ontology_objects WHERE id=$1',[id])).rows[0]:null;
    ...
    const changed=!previous||Object.entries(input.properties).some(([key,value])=>stable(previous.properties[key])!==stable(value));
    if(changed) await writeObservation(db,id,{event_type:previous?'PROPERTIES_CHANGED':'OBJECT_CREATED',source_id:'ontology-ingest',data:{properties:input.properties},provenance:input.provenance.map(p=>({...p,kind:p.kind==='observed'?'imported':p.kind})),dedup_key:randomUUID()});
    return id;
  }
  async merge(db, canonical, duplicate) {
    await mergeIntelligence(db,canonical,duplicate);
  ...
  async putLink(db, raw, validate = true) {
    ...
    const previous=(await db.query('SELECT properties,confidence FROM ontology_links WHERE source_object_id=$1 AND target_object_id=$2 AND link_type=$3 AND valid_from IS NOT DISTINCT FROM $4::timestamptz AND valid_to IS NOT DISTINCT FROM $5::timestamptz',[source,target,raw.link_type,from,to])).rows[0];
    ...
    if(!previous||Object.entries(raw.properties||{}).some(([key,value])=>stable(previous.properties[key])!==stable(value))) {
      const event={event_type:previous?'RELATIONSHIP_CHANGED':'RELATIONSHIP_CREATED',source_id:'ontology-links',data:{link_id:row.id,source_object_id:source,target_object_id:target,link_type:raw.link_type,properties:raw.properties||{}},provenance:evidence.map(p=>({...p,observed_at:p.observed_at instanceof Date?p.observed_at.toISOString():p.observed_at,fetched_at:p.fetched_at instanceof Date?p.fetched_at.toISOString():p.fetched_at})),valid_from:from instanceof Date?from.toISOString():from,valid_to:to instanceof Date?to.toISOString():to};
      event.provenance=event.provenance.map(p=>({...p,kind:p.kind==='observed'?'imported':p.kind}));
      event.dedup_key=randomUUID();
      await writeObservation(db,source,event);if(target!==source)await writeObservation(db,target,event);
    }
    return row.id;
  }
  ...
  async objects(ids, db = this.pool) {
    ...
    return (await db.query(`SELECT o.*, COALESCE((SELECT jsonb_agg(jsonb_build_object('namespace',namespace,'value',value) ORDER BY namespace,value) FROM ontology_identifiers i WHERE i.object_id=o.id),'[]') external_ids,
      COALESCE((SELECT jsonb_agg(p ORDER BY fetched_at DESC) FROM (SELECT provider,source_id,source_record_id,url,observed_at,fetched_at,confidence,confidence_basis,extraction_method,kind,metadata FROM ontology_provenance WHERE object_id=o.id ORDER BY fetched_at DESC LIMIT 50) p),'[]') provenance
