...
const TYPES={OFFICIAL_ALERT:'conflict',HEARD_EXPLOSION:'conflict',CONFLICT_REPORT:'conflict',FIRE:'fire',EARTHQUAKE:'earthquake',SEVERE_WEATHER:'weather',WEATHER:'weather',NEWS_EVENT:'news',CYBER_INDICATOR:'cyber',REFERENCE_LOCATION:'infrastructure'};
...
function intervals(rows){
 ...
 for(const o of [...rows].sort((a,b)=>new Date(b.timeline_at)-new Date(a.timeline_at)||b.id.localeCompare(a.id))){
  ...
  if(moving(d)||d==='infrastructure'||o.event_type==='OFFICIAL_ALERT'){end=Math.min(end,next.get(key)??Infinity);next.set(key,at);}
