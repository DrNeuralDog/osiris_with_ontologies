...
class AirThreatService {
    ...
    async state(raw) {
        ...
        if (saved && saved.until > Date.now())
            return saved.value;
        const healthRows = await this.health.list({ limit: '200', scope: 'provider' });
