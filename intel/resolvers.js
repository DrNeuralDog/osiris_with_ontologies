...
function wdCacheSet(key, data) {
  if (context.getStore()?.warnings?.length) return; // Do not cache a transient source outage for 24 hours.
  if (wdCache.size >= WIKIDATA_CACHE_MAX) {
    const oldest = wdCache.keys().next().value;
    wdCache.delete(oldest);
  }
  const labels = context.getStore();
  if (labels && data.nodes) data = { ...data, nodes: data.nodes.map(n => {
    const qids = labels.get(n.label);
    return qids?.size === 1 ? { ...n, external_ids: [{ namespace: 'wikidata', value: [...qids][0] }] } : n;
  }) };
...
function sourceWarning(message) {
  const ctx = context.getStore(); if (ctx) { ctx.warnings ||= []; ctx.warnings.push(message); }
}
async function sparql(query) {
  ...
  if (!res.ok) { sourceWarning('Wikidata query unavailable'); return []; }
  const json = await res.json();
  const rows = json.results?.bindings || [];
  const ctx = context.getStore();
  if (ctx) for (const row of rows) for (const [key, field] of Object.entries(row)) {
    const qid = field.value?.match(/^https?:\/\/www\.wikidata\.org\/entity\/(Q[1-9][0-9]*)$/)?.[1];
    const label = row[key + 'Label']?.value;
    if (qid && label) {
      const values = ctx.get(label) || new Set(); values.add(qid); ctx.set(label, values);
    }
  }
  return rows;
}
...
function dedup(nodes, links) {
  ...
  return { nodes: uNodes, links: uLinks, fetched_at: new Date().toISOString() };
}
...
async function resolveAircraft(id, properties = {}) {
  ...
  const airlinePrefix = /^[A-Z]{3}[0-9]/.test(callsign) ? callsign.slice(0, 3) : '';
  ...
  if (airlinePrefix && airlinePrefix.length >= 2) {
    ...
    try {
      const results = await sparql(`
        SELECT ?country ?ceo ?parent ?owner ?operator ?flag ?nationality ?employer ?position ?head ?capital ?memberOf ?neighbor ?item ?itemLabel ?countryLabel ?ceoLabel ?parentLabel WHERE {
    ...
    } catch (e) { sourceWarning('Airline ICAO lookup error:'); console.warn('[INTEL] Airline ICAO lookup error:', e.message); }
  }
  ...
  const REG_PREFIXES = {
    ...
    'EC':'Spain','RP':'Philippines','HS':'Thailand','9M':'Malaysia','AP':'Pakistan','PK':'Indonesia',
...
async function resolveVessel(id) {
  ...
  try {
    const results = await sparql(`
      SELECT ?country ?ceo ?parent ?owner ?operator ?flag ?nationality ?employer ?position ?head ?capital ?memberOf ?neighbor ?item ?itemLabel ?ownerLabel ?countryLabel ?operatorLabel ?flagLabel WHERE {
        { ?item wdt:P458 "${id}" . } UNION { ?item wdt:P587 "${id}" . }
  ...
  } catch (e) { sourceWarning('Wikidata vessel error:'); console.warn('[INTEL] Wikidata vessel error:', e.message); }
...
async function resolveCompany(id) {
  ...
  try {
    ...
    const qid = /^Q[1-9][0-9]*$/.test(id) ? id : await wdSearch(id);
    const filter = qid
      ? `VALUES ?item { wd:${qid} }`
      : `?item rdfs:label "${id}"@en . { ?item wdt:P31/wdt:P279* wd:Q4830453 . } UNION { ?item wdt:P31/wdt:P279* wd:Q43229 . }`;
    const results = await sparql(`
      SELECT ?country ?ceo ?parent ?owner ?operator ?flag ?nationality ?employer ?position ?head ?capital ?memberOf ?neighbor ?item ?itemLabel ?countryLabel ?parentLabel ?ceoLabel ?industryLabel WHERE {
        ${filter}
        OPTIONAL { ?item wdt:P17 ?country . }
        OPTIONAL { ?item wdt:P749 ?parent . }
        OPTIONAL { ?item wdt:P169 ?ceo . }
  ...
  } catch (e) { sourceWarning('Wikidata company error:'); console.warn('[INTEL] Wikidata company error:', e.message); }
...
async function resolvePerson(id) {
  ...
  try {
    ...
    const results = await sparql(`
      SELECT ?country ?ceo ?parent ?owner ?operator ?flag ?nationality ?employer ?position ?head ?capital ?memberOf ?neighbor ?item ?itemLabel ?nationalityLabel ?employerLabel ?positionLabel WHERE {
        ${filter}
  ...
  } catch (e) { sourceWarning('Wikidata person error:'); console.warn('[INTEL] Wikidata person error:', e.message); }
...
async function resolveIP(id) {
  ...
  try {
    ...
    const res = await fetchSource(ipApiUrl, { redirect: 'error', signal: AbortSignal.timeout(8000) });
    if (res.ok) {
  ...
  } catch (e) { sourceWarning('ip-api.com error:'); console.warn('[INTEL] ip-api.com error:', e.message); }
  ...
  try {
    ...
    if (!ALLOWED_DOMAINS.has(parsed.hostname)) throw new Error(`Blocked domain: ${parsed.hostname}`);
    const res = await fetchSource(whoisUrl, { redirect: 'error', signal: AbortSignal.timeout(8000) });
    if (res.ok) {
      const json = await res.json();
  ...
  } catch (e) { sourceWarning('RIPEstat WHOIS error:'); console.warn('[INTEL] RIPEstat WHOIS error:', e.message); }
  ...
  try {
    ...
    if (!ALLOWED_DOMAINS.has(parsed.hostname)) throw new Error(`Blocked domain: ${parsed.hostname}`);
    const res = await fetchSource(abuseUrl, { redirect: 'error', signal: AbortSignal.timeout(8000) });
    if (res.ok) {
      const json = await res.json();
  ...
  } catch (e) { sourceWarning('RIPEstat abuse-contact error:'); console.warn('[INTEL] RIPEstat abuse-contact error:', e.message); }
  ...
  try {
    ...
    if (!ALLOWED_DOMAINS.has(parsed.hostname)) throw new Error(`Blocked domain: ${parsed.hostname}`);
    const res = await fetchSource(netUrl, { redirect: 'error', signal: AbortSignal.timeout(8000) });
    if (res.ok) {
      const json = await res.json();
  ...
  } catch (e) { sourceWarning('RIPEstat network-info error:'); console.warn('[INTEL] RIPEstat network-info error:', e.message); }
...
async function resolveCountry(id) {
  ...
  try {
    ...
    const results = await sparql(`
      SELECT ?country ?ceo ?parent ?owner ?operator ?flag ?nationality ?employer ?position ?head ?capital ?memberOf ?neighbor ?item ?itemLabel ?headLabel ?capitalLabel ?population ?gdp
  ...
  } catch (e) { sourceWarning('Wikidata country error:'); console.warn('[INTEL] Wikidata country error:', e.message); }
...
async function resolveLegacy(type, id, props = {}) {
  return context.run(new Map(), async () => {
    const result = await RESOLVERS[type](id, props);
    const labels = context.getStore();
    const nodes = result.nodes.map(n => {
      const qids = labels.get(n.label);
      return qids?.size === 1 ? { ...n, external_ids: [{ namespace: 'wikidata', value: [...qids][0] }] } : n;
    });
    return { ...result, nodes, warnings: [...new Set([...(result.warnings || []), ...(labels.warnings || [])])] };
  });
}
module.exports = { resolveLegacy, RESOLVERS, loadSanctions, sanctionsSearch, sanitizeId, wdSearch, sparql, getStats: () => ({ sanctions_entries: sanctionsIndex.entries.length, sanctions_loaded_at: sanctionsIndex.fetchedAt, wikidata_cache_size: wdCache.size }) };
